...
ExecStart=/usr/bin/python3 $WORK_DIR/telegram_bot.py
Restart=always
RestartSec=10
...
EOF
    ...
    ln -sf $WORK_DIR/bot_manager.sh /usr/local/bin/tgbot
    print_success "管理脚本创建完成，可使用 'tgbot' 命令管理"
}
...
show_usage() {
    ...
    echo ""
    echo "🎮 管理命令:"
    echo "   tgbot start    - 启动Bot服务"
    echo "   tgbot stop     - 停止Bot服务"
    echo "   tgbot restart  - 重启Bot服务"
    echo "   tgbot status   - 查看Bot状态"
    echo "   tgbot logs     - 查看实时日志"
    echo "   tgbot run      - 交互式运行Bot"
    echo ""
    echo "📁 文件位置:"
    echo "   Bot程序: $WORK_DIR/telegram_bot.py"
    echo "   配置文件: $WORK_DIR/bot_config.json"
    echo "   任务文件: $WORK_DIR/tasks.json"
    echo "   日志文件: $WORK_DIR/bot.log"
    echo ""
    echo "🔑 获取API信息:"
    echo "   访问 https://my.telegram.org/apps"
    echo "   创建应用获取 API ID 和 API Hash"
    echo ""
    echo "🤖 创建Bot Token (可选):"
    echo "   与 @BotFather 对话创建Bot"
    echo ""
    echo "========================================"
    print_success "安装完成! 请先运行配置向导"
    echo "========================================"
}

# 主安装流程
main() {
    print_info "开始安装 Telegram 自动签到 Bot..."
